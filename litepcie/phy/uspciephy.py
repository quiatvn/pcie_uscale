...
class USPCIEPHY(Module, AutoCSR):
    def __init__(self, platform, pads, data_width=64, bar0_size=1*MB, cd="sys", pcie_data_width=64):
        ...
        self.msi        = stream.Endpoint([("dat", 32)])
        ...
        if (cd == "pcie") and (data_width == pcie_data_width):
            ...
            self.comb += [
                ...
                cq_pipe_valid.source.connect(self.cmp_source),
            ]
        ...
        if (cd == "pcie") and (data_width == pcie_data_width):
            ...
            self.comb += [
                ...
                rc_pipe_valid.source.connect(self.req_source),
            ]
        ...
        if cd == "pcie":
            ...
            msi_cdc = stream.AsyncFIFO([("dat", 32)], 4)
        ...
        s_axis_rq_tuser = Signal(60)
        s_axis_cc_tuser = Signal(33)        
        m_axis_rc_tuser = Signal(75)
        m_axis_cq_tuser = Signal(85)

        """ s_axis_*.tuser """
        s_axis_rq_first_be = Signal(4)    # ??????????????? check
        s_axis_rq_last_be  = Signal(4)    # ??????????????? check
        s_axis_rq_first_be = s_axis_rq.dat[32:36]
        s_axis_rq_last_be  = s_axis_rq.dat[36:40]

        self.comb += [
            s_axis_rq_tuser[0:].eq(Cat(s_axis_rq_first_be, s_axis_rq_last_be,0)),
            s_axis_cc_tuser.eq(0), #{discontinue, parity (disable)}
            ]

        """ m_axis_*.be """
        m_axis_rc.be = m_axis_rc_tuser[0:pcie_data_width//8]
        m_axis_cq.be = m_axis_cq_tuser[8:pcie_data_width//8 + 8]

        """ s_axis_*.tkeep """
        s_axis_rq_tkeep = Signal(pcie_data_width//32)
        s_axis_cc_tkeep = Signal(pcie_data_width//32)
        m_axis_rc_tkeep = Signal(pcie_data_width//32)
        m_axis_cq_tkeep = Signal(pcie_data_width//32)
        self.comb += [
            s_axis_rq_tkeep[1].eq(s_axis_rq.be[7] | s_axis_rq.be[6] | s_axis_rq.be[5] | s_axis_rq.be[4]),
            s_axis_rq_tkeep[0].eq(s_axis_rq.be[3] | s_axis_rq.be[2] | s_axis_rq.be[1] | s_axis_rq.be[0]),
            s_axis_cc_tkeep[1].eq(s_axis_cc.be[7] | s_axis_cc.be[6] | s_axis_cc.be[5] | s_axis_cc.be[4]),
            s_axis_cc_tkeep[0].eq(s_axis_cc.be[3] | s_axis_cc.be[2] | s_axis_cc.be[1] | s_axis_cc.be[0]),
            ]

        if (pcie_data_width == 128):
            self.comb += [
                s_axis_rq_tkeep[3].eq(s_axis_rq.be[15] | s_axis_rq.be[14] | s_axis_rq.be[13] | s_axis_rq.be[12]),
                s_axis_rq_tkeep[2].eq(s_axis_rq.be[11] | s_axis_rq.be[10] | s_axis_rq.be[9] | s_axis_rq.be[8]),
                s_axis_cc_tkeep[3].eq(s_axis_cc.be[15] | s_axis_cc.be[14] | s_axis_cc.be[13] | s_axis_cc.be[12]),
                s_axis_cc_tkeep[2].eq(s_axis_cc.be[11] | s_axis_cc.be[10] | s_axis_cc.be[9] | s_axis_cc.be[8]),
                ]
        ...
        self.comb += [
            m_axis_cq.first.eq(m_axis_cq_tuser[40]),
            m_axis_cq.last.eq (m_axis_cq_tlast),
            m_axis_rc.first.eq(m_axis_rc_tuser[32]),
        ...
        self.pcie_phy_params = dict(
            ...
            p_KEEP_WIDTH                                 = pcie_data_width//32,
            ...
            i_s_axis_rq_tkeep                            = s_axis_rq_tkeep,
            ...
            o_m_axis_cq_tkeep                            = m_axis_cq_tkeep,
            ...
            o_m_axis_rc_tkeep                            = m_axis_rc_tkeep,
            ...
            i_s_axis_cc_tkeep                            = s_axis_cc_tkeep,
